...
const PublicEventsPage: React.FC = () => {
  ...
  const getEventTypeColor = (type: string) => {
    switch (type) {
      case 'virtual': return 'bg-purple-100 dark:bg-purple-900/20 text-purple-800 dark:text-purple-400';
      case 'physical': return 'bg-orange-100 dark:bg-orange-900/20 text-orange-800 dark:text-orange-400';
      case 'hybrid': return 'bg-blue-100 dark:bg-blue-900/20 text-blue-800 dark:text-blue-400';
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'business': return 'bg-blue-100 dark:bg-blue-900/20 text-blue-800 dark:text-blue-400';
      case 'technology': return 'bg-cyan-100 dark:bg-cyan-900/20 text-cyan-800 dark:text-cyan-400';
      case 'entertainment': return 'bg-pink-100 dark:bg-pink-900/20 text-pink-800 dark:text-pink-400';
      case 'education': return 'bg-green-100 dark:bg-green-900/20 text-green-800 dark:text-green-400';
      case 'health': return 'bg-emerald-100 dark:bg-emerald-900/20 text-emerald-800 dark:text-emerald-400';
      default: return 'bg-gray-100 dark:bg-gray-900/20 text-gray-800 dark:text-gray-400';
    }
  };
      ...
      <div className="bg-gradient-to-r from-blue-700 to-cyan-700 rounded-xl p-8 text-white">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-lg md:text-xl mb-8 opacity-90">
            Find and attend events that match your interests and connect with like-minded people
          </p>
          ...
          <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredEvents.map((event) => (
                  <Link 
                    key={event.id} 
                    to={`/events/${event.id}`}
                    className="bg-white dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-800 overflow-hidden hover:shadow-lg transition-shadow"
                  >
                    {event.image && (
              ...
              <div className="space-y-4">
                {filteredEvents.map((event) => (
                  <Link 
                    key={event.id} 
                    to={`/events/${event.id}`}
                    className="block bg-white dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-800 overflow-hidden hover:shadow-lg transition-shadow"
                  >
            ...
            {viewMode === 'calendar' && (
              <div className="bg-white dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-800 p-6">
                ...
                {Array.from(new Set(filteredEvents.map(event => event.date))).sort().map(date => (
                      ...
                      {filteredEvents
                        ...
                        .map(event => (
                          <Link 
                            key={event.id} 
                            to={`/events/${event.id}`}
                            className="flex items-start p-4 bg-gray-50 dark:bg-zinc-800 rounded-lg hover:bg-gray-100 dark:hover:bg-zinc-700 transition-colors"
                          >
                            ...
                            <div className="ml-4 flex-1">
                              ...
                              <div className="flex flex-wrap gap-2 mt-1 mb-2">
                                <span className={`inline-flex px-2 py-0.5 text-xs font-semibold rounded-full ${getEventTypeColor(event.type)}`}>
                                  {event.type}
                                </span>
                                <span className={`inline-flex px-2 py-0.5 text-xs font-semibold rounded-full ${getCategoryColor(event.category)}`}>
                                  {event.category}
                                </span>
                              </div>
                              <div className="flex items-center space-x-2 text-sm text-gray-600 dark:text-zinc-400">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {categories.filter(c => c.id !== 'all').map(category => (
            <Link
              ...
              className={`p-6 rounded-xl border border-gray-200 dark:border-zinc-800 hover:shadow-md transition-shadow text-center ${
                getCategoryColor(category.id).replace('text-', 'hover:bg-').replace('dark:text-', 'dark:hover:bg-').replace('800', '50').replace('400', '900/10')
              }`}
            >
