...
import { useNotifications, useConfirmation } from '@/hooks';

import { Popover } from '@/components/feedbacks';
import { LoadingButton } from '@/components/user-inputs/buttons';
import { Modal } from '@/components/user-inputs/modals';
import { InviteAdminForm } from '@/components/user-inputs/forms';
...
const Admins: React.FC = () => {
  ...
  const filteredSubAdmins = mockAdmins.filter((admin) => {
    const matchesSearch =
      admin.name.toLowerCase().includes(searchTerm.toLowerCase()) || admin.email.toLowerCase().includes(searchTerm.toLowerCase()) || admin.role.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-gray-200 dark:border-zinc-800">
          <div className="text-2xl font-bold text-gray-900 dark:text-white mb-1">{mockAdmins.length}</div>
          <div className="text-sm text-gray-500 dark:text-zinc-400">Total Sub-Admins</div>
        </div>
        <div className="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-gray-200 dark:border-zinc-800">
          <div className="text-2xl font-bold text-gray-900 dark:text-white mb-1">{mockAdmins.filter((a) => a.status === 'active').length}</div>
          <div className="text-sm text-gray-500 dark:text-zinc-400">Active</div>
        </div>
        <div className="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-gray-200 dark:border-zinc-800">
          <div className="text-2xl font-bold text-gray-900 dark:text-white mb-1">{mockAdmins.filter((a) => a.permissions.includes('events.manage')).length}</div>
          <div className="text-sm text-gray-500 dark:text-zinc-400">Event Managers</div>
        </div>
        <div className="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-gray-200 dark:border-zinc-800">
          <div className="text-2xl font-bold text-gray-900 dark:text-white mb-1">{mockAdmins.filter((a) => a.status === 'inactive').length}</div>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-1">
                      ...
                      {admin.permissions.length > 2 && (
                        <span className="inline-flex px-2 py-1 text-xs bg-gray-100 dark:bg-zinc-800 text-gray-700 dark:text-zinc-300 rounded">+{admin.permissions.length - 2} more</span>
                      )}
                    </div>
                  </td>
                    ...
                    >
                      ...
                      {admin.status === 'active' ? (
                        <PopoverMenuItem
                          ...
                          label="Deactivate"
                          onClick={() => handleSubAdminAction('deactivate', admin)}
                        ...
                        <PopoverMenuItem
                          icon={<Shield className="w-4 h-4" />}
                          label="Activate"
                          onClick={() => handleSubAdminAction('activate', admin)}
                      ...
                      <PopoverMenuItem
                        ...
                        label="Remove"
                        onClick={() => handleSubAdminAction('delete', admin)}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  ...
                  { id: 'customers.manage', name: 'Manage Customers' },
