...
const Customers: React.FC = () => {
  ...
  const filteredCustomers = mockCustomers.filter((customer) => {
    const matchesSearch = customer.name.toLowerCase().includes(searchTerm.toLowerCase()) || customer.email.toLowerCase().includes(searchTerm.toLowerCase()) || customer.company.toLowerCase().includes(searchTerm.toLowerCase());
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-zinc-800">
              {filteredCustomers.map((customer) => (
                <tr key={customer.id} className="hover:bg-gray-50 dark:hover:bg-zinc-800 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getCheckInColor(customer.checkInStatus)}`}>{customer.checkInStatus === "checked-in" ? "Checked In" : "Not Checked In"}</span>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    ...
                    >
                      ...
                      {customer.checkInStatus === "not-checked-in" && (
                        <PopoverMenuItem icon={<UserCheck className="w-4 h-4" />} label="Check In" onClick={() => handleCustomerAction("check-in", customer)} className="text-emerald-600 dark:text-emerald-400" />
                      )}
                      <hr className="my-1 border-gray-200 dark:border-zinc-700" />
                      <PopoverMenuItem icon={<UserX className="w-4 h-4" />} label="Cancel Registration" onClick={() => handleCustomerAction("cancel", customer)} className="text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20" />
                    </Popover>
                  </td>
                </tr>
              ))}
