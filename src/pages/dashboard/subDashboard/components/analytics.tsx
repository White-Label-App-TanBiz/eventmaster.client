...
const Analytics: React.FC = () => {
  ...
  // Base metrics that will be adjusted by period
  const baseMetrics = [
    {
  ...
  // Apply period filtering to metrics
  const periodMetrics = baseMetrics.map((metric) => ({
    ...metric,
    adjustedAmount: getPeriodData(metric.baseAmount, period),
  }));

  const baseTopPerformers = [
    { name: "TechCorp Events", revenue: 12450, growth: "+34%" },
    { name: "Innovate Solutions", revenue: 8920, growth: "+28%" },
    { name: "Global Events Ltd", revenue: 7340, growth: "+22%" },
    { name: "Event Masters", revenue: 6180, growth: "+19%" },
    { name: "ProEvent Agency", revenue: 5760, growth: "+15%" },
  ];
  ...
  const getChartData = () => {
    const baseData = [12, 19, 15, 25, 18, 30, 28, 35, 32, 42, 38, 45];
  ...
  // Period-specific analytics breakdown
  const getAnalyticsBreakdown = () => {
    ...
    return {
      subscriptionRevenue: getPeriodData(baseBreakdown.subscriptionRevenue, period),
      transactionFees: getPeriodData(baseBreakdown.transactionFees, period),
      setupFees: getPeriodData(baseBreakdown.setupFees, period),
      dailyActiveUsers: getPeriodData(baseBreakdown.dailyActiveUsers, period),
      sessionDuration: baseBreakdown.sessionDuration,
      bounceRate: baseBreakdown.bounceRate,
      conversionRate: baseBreakdown.conversionRate,
      totalEvents: getPeriodData(baseBreakdown.totalEvents, period),
      virtualEvents: getPeriodData(baseBreakdown.virtualEvents, period),
      physicalEvents: getPeriodData(baseBreakdown.physicalEvents, period),
      avgAttendance: baseBreakdown.avgAttendance,
    };
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col space-y-4 sm:space-y-0 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="grid grid-cols-2 gap-3 sm:flex sm:space-x-3 sm:gap-0">
          ...
          <div className="col-span-1">
            <button className="w-full px-4 py-2 bg-gradient-to-r from-blue-700 to-cyan-700 hover:from-blue-800 hover:to-cyan-800 text-white rounded-lg text-sm transition-all shadow-lg hover:shadow-xl whitespace-nowrap">Export Report</button>
          </div>
        </div>
      </div>
          ...
          return (
            <div key={index} className="bg-white dark:bg-zinc-900 p-4 lg:p-6 rounded-xl border border-gray-200 dark:border-zinc-800">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className={`text-xs font-medium px-2 py-1 rounded-full ${metric.trend === "up" ? "bg-emerald-100 dark:bg-emerald-900/20 text-emerald-800 dark:text-emerald-400" : "bg-red-100 dark:bg-red-900/20 text-red-800 dark:text-red-400"}`}>
