...
const ClientAdmins: React.FC = () => {
  ...
  // Simple client statistics without period filtering
  ...
  const handleClientAction = async (action: string, client: any) => {
    const actionKey = `${action}-${client.id}`;
    ...
    try {
      ...
      showError("Action failed", `Failed to ${action} customer. Please try again.`);
    }
  };
  ...
  const handleExport = async () => {
    await withLoading("export", async () => {
      ...
      showSuccess("Export completed", "Customer data has been exported successfully.");
    });
  };
  ...
  const handleSaveEdit = async (updatedData: any) => {
    await withLoading("save-edit", async () => {
      ...
      showSuccess("Customer updated", `${updatedData.name || selectedClientForModal.name}'s information has been updated.`);
      closeModal();
    });
  };
    ...
    switch (modalType) {
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <div className="flex items-center space-x-2 mt-1">
                  ...
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getPaymentStatusColor(selectedClientForModal.paymentStatus)}`}>{selectedClientForModal.paymentStatus}</span>
                </div>
              </div>
            </div>
      ...
      case "edit":
        return (
          <div className="space-y-6">
            <div>
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Edit Customer Information</h3>
              <p className="text-gray-600 dark:text-zinc-400 mb-6">Update customer details for {selectedClientForModal.name}</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="mt-1">
                    <span className={`inline-flex px-3 py-1 text-sm font-semibold rounded-full ${getPaymentStatusColor(selectedClientForModal.paymentStatus)}`}>{selectedClientForModal.paymentStatus}</span>
                  </div>
                </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-gray-200 dark:border-zinc-800">
          ...
          <div className="text-sm text-gray-500 dark:text-zinc-400">Active Customers</div>
        </div>
        <div className="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-gray-200 dark:border-zinc-800">
          <div className="text-2xl font-bold text-gray-900 dark:text-white mb-1">{clientStats.newClients}</div>
          <div className="text-sm text-gray-500 dark:text-zinc-400">New Customers</div>
        </div>
        <div className="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-gray-200 dark:border-zinc-800">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between">
                        ...
                        >
                          ...
                          <PopoverMenuItem icon={<Edit className="w-4 h-4" />} label="Edit Customer" onClick={() => handleClientAction("edit", client)} />
                          ...
                          <PopoverMenuItem icon={<Trash2 className="w-4 h-4" />} label="Delete Customer" onClick={() => handleClientAction("delete", client)} className="text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20" />
                        </Popover>
                      </div>
