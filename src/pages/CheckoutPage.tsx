...
const CheckoutPage: React.FC = () => {
  ...
  useEffect(() => {
    if (planId) {
      const foundPlan = mockProductPlans.find((p) => p.id === planId);
      if (foundPlan) {
        setPlan(foundPlan);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await withLoading("checkout", async () => {
      ...
      showSuccess("Subscription successful!", `Welcome to ${plan.name}! Your account has been created and you can start using EventMaster immediately.`);

      // Redirect to thank you page with subscription details
      setTimeout(() => {
        navigate("/thank-you", {
          state: {
            subscriptionDetails: {
                  ...
                  <div className="mb-6">
                    <div className="flex space-x-4">
                      <label className="flex items-center space-x-3 cursor-pointer">
                        <input type="radio" value="card" checked={paymentMethod === "card"} onChange={(e) => setPaymentMethod(e.target.value as "card" | "paypal")} className="w-4 h-4 text-blue-700 focus:ring-blue-700" />
                        <span className="text-sm font-medium text-gray-900 dark:text-white">Credit Card</span>
                      </label>
                      <label className="flex items-center space-x-3 cursor-pointer">
                        <input type="radio" value="paypal" checked={paymentMethod === "paypal"} onChange={(e) => setPaymentMethod(e.target.value as "card" | "paypal")} className="w-4 h-4 text-blue-700 focus:ring-blue-700" />
              ...
              <div className="bg-white dark:bg-zinc-900 p-6 rounded-xl border border-gray-200 dark:border-zinc-800">
                <div className="space-y-4">
                  <label className="flex items-start space-x-3 cursor-pointer">
                    <input type="checkbox" checked={formData.agreeToTerms} onChange={(e) => handleInputChange("agreeToTerms", e.target.checked)} className="w-4 h-4 text-blue-700 focus:ring-blue-700 mt-1" required />
                  ...
                  <label className="flex items-start space-x-3 cursor-pointer">
                    <input type="checkbox" checked={formData.subscribeToUpdates} onChange={(e) => handleInputChange("subscribeToUpdates", e.target.checked)} className="w-4 h-4 text-blue-700 focus:ring-blue-700 mt-1" />
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      <label className="flex items-center justify-between p-3 border border-gray-200 dark:border-zinc-700 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-zinc-800">
                        <div className="flex items-center space-x-3">
                          <input type="radio" value="monthly" checked={billingCycle === "monthly"} onChange={(e) => setBillingCycle(e.target.value as "monthly" | "yearly")} className="w-4 h-4 text-blue-700 focus:ring-blue-700" />
                      ...
                      <label className="flex items-center justify-between p-3 border border-gray-200 dark:border-zinc-700 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-zinc-800">
                        <div className="flex items-center space-x-3">
                          <input type="radio" value="yearly" checked={billingCycle === "yearly"} onChange={(e) => setBillingCycle(e.target.value as "monthly" | "yearly")} className="w-4 h-4 text-blue-700 focus:ring-blue-700" />
                          <div>
